...
def test_order_id_only_message_detected_as_order_status() -> None:
    agent = CustomerServiceAgent()
    intent, _ = agent.detect_intent("2026001")
    assert intent == "order_status"


def test_order_id_with_previous_after_sales_stays_after_sales() -> None:
    agent = CustomerServiceAgent()
    intent, _ = agent.detect_intent("2026001", previous_intent="after_sales_policy")
    assert intent == "after_sales_policy"
